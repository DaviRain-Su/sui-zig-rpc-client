...
```
src/cli/
├── root.zig         # 模块入口 (2,500 行)
├── types.zig        # 命令枚举 (7,700 行, 15 测试)
├── parsed_args.zig  # 参数结构体 (9,000 行, 15 测试)
├── parser.zig       # 参数解析器 (29,300 行, 15 测试)
├── help.zig         # 帮助系统 (19,000 行, 6 测试)
├── validator.zig    # 参数验证 (14,000 行, 20 测试)
├── utils.zig        # 工具函数 (9,900 行, 15 测试)
├── integration.zig  # 集成层 (13,400 行, 5 测试)
└── e2e_test.zig     # 端到端测试 (12,100 行, 16 测试)
```
