...
- `request build/dry-run/sponsor/sign/send` 现在也会在有可写 request-state 路径时自动写入本地 lifecycle entry：分别对应 `built`、`resolved`、`sponsored`、`signed`、`submitted/confirmed/failed`。如果显式给了 `--correlation-id`，这些阶段会复用同一个 request entry，而不是各写一条孤立记录。
