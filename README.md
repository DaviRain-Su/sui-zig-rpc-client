...
- `wallet-core v1` 收口清单：[`docs/wallet-core-v1-checklist.md`](/Users/davirian/dev/zig/sui-zig-rpc-client/docs/wallet-core-v1-checklist.md)
- `wallet-web` 架构规划：[`docs/wallet-web-architecture.md`](/Users/davirian/dev/zig/sui-zig-rpc-client/docs/wallet-web-architecture.md)
- `wallet-core v1` release gate 脚本：[`scripts/wallet_core_v1_release_gate.sh`](/Users/davirian/dev/zig/sui-zig-rpc-client/scripts/wallet_core_v1_release_gate.sh)
- 建议执行：
...
如果你要把当前仓库当成 `wallet-core v1` 做一次统一 release gate，而不是手工分别跑测试和 smoke，现在直接用：
...
- `zig build move-fixture-test`
- `hashi_publish_smoke`
- `hashi_finish_publish_smoke`
- `hashi_deposit_smoke`
- Cetus mainnet 轻量 sanity：
  - `sui_getNormalizedMoveFunction`
  - `object get cetus_clmm_global_config_mainnet --summarize`

如果当前环境没有可用的主网 RPC，也可以显式跳过 live sanity，只跑本地 gate：
...
- `request sign`: 直接对 request artifact 或 request-shaped 输入附加 signer/provider 审批，并输出 execute payload；复用现有本地 programmable builder 和 `tx payload` 路径。现在也支持 `--session <selector|label|session-id|0xaddress>`，并会把 delegated session 注入 authorizer/local signer 执行路径，而不只停在 artifact metadata。执行侧 remote/passkey/future-wallet authorizer 现在也会收到和 artifact 一致的 merged `policy_json` / `delegated_session_json`，不会再只看到裸 request。
- `request send`: 直接对 request artifact 或 request-shaped 输入执行发送，复用现有本地 programmable builder 和 signer/provider 路径。现在也支持 `--session <selector|label|session-id|0xaddress>`，命中 `local_signer` session 时即使没有显式 `--from-keystore/--signer/--provider` 也能直接合成执行侧 signer；命中 provider 路径时同样会把 merged session/policy contract 传给 authorizer。
...
- `request build/dry-run/sponsor/sign/send` 现在也会在有可写 request-state 路径时自动写入本地 lifecycle entry：分别对应 `built`、`resolved`、`sponsored`、`signed`、`submitted/confirmed/failed`；如果 `request sign/send` 命中了 session/provider challenge prompt，则同样会写成 `challenge_required`；如果 `request sign/send` 直接失败，也会把 `failed + last_error` 回写到同一条 entry，而不是只把错误抛给终端。如果显式给了 `--correlation-id`，这些阶段会复用同一个 request entry，而不是各写一条孤立记录。
...
- `request status <entry-id|digest>`: 复用交易状态读取路径查询 request 执行结果；支持 `--summarize`、`--observe`、`--poll-ms`、`--confirm-timeout-ms`。如果输入命中了本地 `request_state.json` 条目，CLI 会优先读取该条目的已知 digest，并把最新 `confirmed/failed/submitted` 状态回写到同一条 request entry。
