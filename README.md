...
- `wallet balance [selector]`: 聚合钱包 coin 余额；支持 `--coin-type`、`--limit`、`--all`。默认无 `--limit` 时会直接走 `suix_getAllBalances`，单币种过滤走 `suix_getBalance`；只有显式给 `--limit` 时才回到 paged coin scan。`--all` 保留为兼容 alias；未给 selector 时同样优先用 active wallet selector。
