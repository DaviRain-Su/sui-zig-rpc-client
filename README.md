...
- `parameters[*].shared_object_candidates`: 对已经 concrete 的 shared object 类型，CLI 会尝试从 recent module events 里抽取 object id，再用 `object get --summarize` 过滤出类型匹配的 shared object 候选；当前会连续扫描多页 recent events，而不只看第一页；如果 owner-context 已经发现到的 owned object 内容里还引用了额外 shared object，这些候选也会一起聚合进来，而不是和 events 发现互斥覆盖
