...
对于 direct-signature 的 `tx payload` / `tx send` programmable 路径，如果已经给了显式 `--sender` 和 `--gas-budget`，但没有显式 `--gas-payment`，CLI 现在会优先在本地 builder 路径里自动挑一枚 gas coin，而不是直接退回 `unsafe_moveCall` / `unsafe_batchTransaction`。
