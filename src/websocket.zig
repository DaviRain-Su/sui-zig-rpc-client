...
pub const SuiEventSubscriber = struct {
    ...
    pub fn subscribeToEvents(self: *Self, filter: EventFilter) !void {
        const request = try std.fmt.allocPrint(self.allocator, "{{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"suix_subscribeEvent\",\"params\":[{s}]}}", .{filter.toJson()});
        defer self.allocator.free(request);
    ...
    pub fn subscribeToTransactions(self: *Self, filter: TransactionFilter) !void {
        const request = try std.fmt.allocPrint(self.allocator, "{{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"suix_subscribeTransaction\",\"params\":[{s}]}}", .{filter.toJson()});
        defer self.allocator.free(request);
