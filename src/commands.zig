...
fn implicitLocalGasPaymentFallbackMinBalance(
    ...
    const has_supported_local_signer_source = if (provider) |value|
        ...
        hasRealBuilderSignerSource(args) or args.tx_build_sender != null;
...
test "runCommand move function with --summarize caches shared event discovery across identical shared params" {
    ...
    try testing.expectEqual(@as(usize, 2), state.query_events_requests);
...
test "runCommand move function with --summarize uses queried package for shared object event discovery and candidates" {
    ...
    defer _ = gpa.deinit();
    const allocator = gpa.allocator();
    var query_events_requests: usize = 0;
...
test "runCommand move function with --summarize discovers shared candidates from type module events" {
    const testing = std.testing;
    ...
    const callback = struct {
        fn call(context: *anyopaque, alloc: std.mem.Allocator, req: RpcRequest) ![]u8 {
            ...
            if (std.mem.eql(u8, req.method, "sui_getNormalizedMoveFunction")) {
                return alloc.dupe(
                    u8,
                    "{\"result\":{\"visibility\":\"Public\",\"isEntry\":false,\"typeParameters\":[],\"parameters\":[{\"MutableReference\":{\"Struct\":{\"address\":\"0x2a\",\"module\":\"pool\",\"name\":\"Pool\",\"typeParams\":[{\"Struct\":{\"address\":\"0x2\",\"module\":\"sui\",\"name\":\"SUI\",\"typeParams\":[]}},{\"Struct\":{\"address\":\"0x2\",\"module\":\"sui\",\"name\":\"SUI\",\"typeParams\":[]}}]}}}],\"return\":[]}}",
                );
            }
            if (std.mem.eql(u8, req.method, "suix_queryEvents")) {
                request_count.* += 1;
                if (request_count.* == 1) {
                    std.debug.assert(std.mem.eql(
                        u8,
                        req.params_json,
                        "[{\"MoveModule\":{\"package\":\"0x2a\",\"module\":\"router\"}},null,20,true]",
                    ));
                    return alloc.dupe(
                        u8,
                        "{\"result\":{\"data\":[],\"hasNextPage\":false}}",
                ...
                std.debug.assert(std.mem.eql(
                    u8,
                    req.params_json,
                    "[{\"MoveModule\":{\"package\":\"0x2a\",\"module\":\"pool\"}},null,20,true]",
                ));
                return alloc.dupe(
                    u8,
                    "{\"result\":{\"data\":[{\"id\":{\"txDigest\":\"0xtype-event\",\"eventSeq\":\"1\"},\"parsedJson\":{\"pool_id\":\"0xpool-type-module\"}}],\"hasNextPage\":false}}",
                );
            }
            if (std.mem.eql(u8, req.method, "sui_getTransactionBlock")) {
                return alloc.dupe(u8, "{\"result\":{\"objectChanges\":[]}}");
            ...
            std.debug.assert(std.mem.eql(u8, req.method, "sui_getObject"));
            std.debug.assert(std.mem.indexOf(u8, req.params_json, "\"0xpool-type-module\"") != null);
            if (std.mem.indexOf(u8, req.params_json, "\"showContent\":true") != null) {
                return alloc.dupe(
                    u8,
    ...
    const parameter = parsed.value.object.get("parameters").?.array.items[0].object;
    const shared_event_argv = parameter.get("shared_object_event_query_argv").?.array.items;
    try testing.expectEqualStrings("router", shared_event_argv[4].string);
    const shared_candidates = parameter.get("shared_object_candidates").?.array.items;
    try testing.expectEqual(@as(usize, 1), shared_candidates.len);
    try testing.expectEqualStrings("0xpool-type-module", shared_candidates[0].object.get("object_id").?.string);
...
test "runCommand tx_payload with commands builds execute payload" {
    ...
    const Counts = struct { coin: usize = 0, object: usize = 0, unsafe: usize = 0 };
    var counts = Counts{};
    const callback = struct {
        fn call(context: *anyopaque, alloc: std.mem.Allocator, req: RpcRequest) ![]u8 {
            const state = @as(*Counts, @ptrCast(@alignCast(context)));
    ...
    try testing.expect(items[0].string.len > 0);
...
test "runCommand tx_payload move-call builds execute payload" {
    ...
    const Counts = struct { gas_coin: usize = 0, normalized: usize = 0, object: usize = 0, unsafe: usize = 0 };
    var counts = Counts{};
    const callback = struct {
        fn call(context: *anyopaque, alloc: std.mem.Allocator, req: RpcRequest) ![]u8 {
            const state = @as(*Counts, @ptrCast(@alignCast(context)));
    ...
    const payload = try std.json.parseFromSlice(std.json.Value, allocator, output.items, .{});
    defer payload.deinit();
    try testing.expectEqual(@as(usize, 1), counts.gas_coin);
    try testing.expectEqual(@as(usize, 1), counts.normalized);
    try testing.expectEqual(@as(usize, 1), counts.object);
    try testing.expectEqual(@as(usize, 0), counts.unsafe);
    try testing.expect(payload.value == .array);
    ...
    const callback = struct {
        fn call(context: *anyopaque, alloc: std.mem.Allocator, req: RpcRequest) ![]u8 {
            ...
            if (std.mem.eql(u8, req.method, "sui_getNormalizedMoveFunction")) {
                ...
                return alloc.dupe(
                    u8,
                    "{\"result\":{\"visibility\":\"Public\",\"isEntry\":true,\"typeParameters\":[],\"parameters\":[{\"Struct\":{\"address\":\"0x2\",\"module\":\"counter\",\"name\":\"Counter\",\"typeParams\":[]}}, {\"MutableReference\":{\"Struct\":{\"address\":\"0x2\",\"module\":\"tx_context\",\"name\":\"TxContext\",\"typeParams\":[]}}}],\"return\":[]}}",
                );
            }
            if (std.mem.eql(u8, req.method, "sui_getObject")) {
                ctx.counts.object += 1;
...
test "runCommand tx_dry_run move-call with omitted gas payment keeps local programmable builder path" {
    const testing = std.testing;
...
test "runCommand tx_dry_run request artifact accepts base58 digests in local object refs" {
    const testing = std.testing;
    ...
    var args = try cli.parseCliArgs(allocator, &.{
        ...
        "dry-run",
        "--request",
        "{\"commands\":[{\"kind\":\"TransferObjects\",\"objects\":[\"select:{\\\"kind\\\":\\\"object_input\\\",\\\"objectId\\\":\\\"0x1111111111111111111111111111111111111111111111111111111111111111\\\",\\\"inputKind\\\":\\\"imm_or_owned\\\",\\\"version\\\":7,\\\"digest\\\":\\\"11111111111111111111111111111111\\\"}\"],\"address\":\"0x2222222222222222222222222222222222222222222222222222222222222222\"}],\"sender\":\"0x3333333333333333333333333333333333333333333333333333333333333333\",\"gasBudget\":1200,\"gasPrice\":8,\"gasPayment\":[{\"objectId\":\"0x4444444444444444444444444444444444444444444444444444444444444444\",\"version\":\"3\",\"digest\":\"11111111111111111111111111111111\"}],\"summarize\":true}",
    });
    defer args.deinit(allocator);
...
test "runCommand tx_dry_run treats omitted option fields in nested generic pure structs as none" {
    const testing = std.testing;
    ...
    var args = cli.ParsedArgs{
        ...
        .tx_build_function = "submit_complex",
        .tx_build_type_args = "[\"0x2::sui::SUI\"]",
        .tx_build_args = "[{\"owner\":\"0x1111111111111111111111111111111111111111111111111111111111111111\",\"spending\":7,\"weights\":[1,2]},{\"owner\":\"0x2222222222222222222222222222222222222222222222222222222222222222\",\"spending\":11,\"weights\":[3]},[4,5]]",
        .tx_build_sender = "0x123",
        .tx_build_gas_budget = 1200,
        .tx_build_gas_price = 8,
        .tx_build_gas_payment = "[{\"objectId\":\"0x999\",\"version\":\"3\",\"digest\":\"0x3333333333333333333333333333333333333333333333333333333333333333\"}]",
...
test "runCommand tx_build move-call with emit-tx-block includes gas metadata" {
    ...
    const Counts = struct { coin: usize = 0, normalized: usize = 0, object: usize = 0, unsafe: usize = 0 };
    var counts = Counts{};
    const callback = struct {
        fn call(context: *anyopaque, alloc: std.mem.Allocator, req: RpcRequest) ![]u8 {
            const state = @as(*Counts, @ptrCast(@alignCast(context)));
...
test "runCommand tx_build programmable resolves selected argument tokens into tx-block output" {
    ...
    const Counts = struct { owned: usize = 0, coin: usize = 0, normalized: usize = 0, object: usize = 0 };
    var counts = Counts{};
    const callback = struct {
        fn call(context: *anyopaque, alloc: std.mem.Allocator, req: RpcRequest) ![]u8 {
            const state = @as(*Counts, @ptrCast(@alignCast(context)));
            ...
            if (std.mem.eql(u8, req.method, "sui_getObject")) {
                ...
                return alloc.dupe(
                    u8,
                    "{\"result\":{\"data\":{\"objectId\":\"0xfedcba\",\"version\":\"9\",\"digest\":\"0x2222222222222222222222222222222222222222222222222222222222222222\",\"owner\":{\"AddressOwner\":\"0xowner\"}}}}",
                );
            }
            std.debug.assert(std.mem.eql(u8, req.method, "suix_getCoins"));
            state.coin += 1;
...
test "runCommand tx_build move-call with emit-tx-block and omitted gas payment keeps local programmable builder path" {
    const testing = std.testing;
...
test "runCommand tx_simulate move-call with omitted gas payment keeps local programmable builder path" {
    const testing = std.testing;
    ...
    const Counts = struct { coin: usize = 0, normalized: usize = 0, object: usize = 0, inspect: usize = 0, unsafe: usize = 0 };
    var counts = Counts{};
    var params_text: ?[]const u8 = null;
